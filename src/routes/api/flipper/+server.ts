...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        // Get the stance settings node (assuming 'DefaultStanceSettings' and 'OnButtonStanceSettings' are the nodes you need)
        const stanceSettings = parsedXml.StanceSettings;
        if (stanceSettings) {
            if (stanceSettings.DefaultStanceSettings) {
                swapFootSettings(stanceSettings.DefaultStanceSettings[0]);
            }
            if (stanceSettings.OnButtonStanceSettings) {
                swapFootSettings(stanceSettings.OnButtonStanceSettings[0]);
            }
        }
