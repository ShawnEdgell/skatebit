...
<div use:tocCrawler={{ mode: 'generate', scrollTarget: '#page', key: tabIndex }}>
    <article>
        <div class="header">
            ...
            <span class="badge variant-filled-tertiary mb-2 cursor-pointer {($activeGames.includes('Skater XL') ? 'active bg-blue-500' : 'inactive bg-gray-500')}" on:click={() => toggleGame('Skater XL')}>Skater XL</span>
            <span class="badge variant-filled-error mb-2 cursor-pointer {($activeGames.includes('Session') ? 'active bg-blue-500' : 'inactive bg-gray-500')}" on:click={() => toggleGame('Session')}>Session</span>
            <span class="badge variant-filled-warning mb-2 cursor-pointer {($activeGames.includes('Skate.') ? 'active bg-blue-500' : 'inactive bg-gray-500')}" on:click={() => toggleGame('Skate.')}>Skate.</span>
            <h1>News</h1>
        ...
        {#each allNewsItems as item}
            {#if $activeGames.includes(item.game)}
                <div>
                    ...
                    <iframe src={item.videoUrl} title={item.videoTitle} frameborder="0" allowfullscreen loading="lazy"></iframe>
                </div>
            {/if}
        {/each}
    </article>
