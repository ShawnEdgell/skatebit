<script lang="ts">
	...
	const modderDiscords: DiscordServer[] = [
		{
...
<div use:tocCrawler={{ mode: 'generate', scrollTarget: '#page' }}>
	<article>
		<div class="header">
			...
			<h1>Helpful Links</h1>
			<p>
				Explore our curated list of helpful links for Skater XL modding, map discovery, and gear
				enhancements.
			</p>
			<hr class="!border-t-2" />
		</div>
								...
								<tr>
									<td>
										<a href={server.link} target="_blank">
											<p class="text-primary-500">{server.name}</p>
										</a>
									</td>
									<td>{server.owner}</td>
								</tr>
