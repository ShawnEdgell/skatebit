...
export const load: PageServerLoad = async ({ locals: { supabase } }) => {
    ...
    const submissionsWithReactions = submissions.map(submission => {
        const reaction_count = submission.streets_reactions ? submission.streets_reactions.length : 0;
...
export const actions: Actions = {
    addSubmission: async ({ request, locals: { supabase, getSession } }) => {
        ...
        const { error } = await supabase.from('streets_edits').insert({
    ...
    addReaction: async ({ request, locals: { supabase, getSession } }) => {
        ...
        const { error } = await supabase.from('streets_reactions').upsert({
