...
<!-- <script lang="ts">
	...
	import { decalFix } from '$lib/data/modData';
	...
	async function fetchModData(version: string) {
		const res = await fetch(`https://skatebit-api.vercel.app/api/mods/${version}`);
		if (!res.ok) return [];
		return await res.json();
	}

	onMount(async () => {
		alphaMods = await fetchModData('1228');
		publicMods = await fetchModData('12104');
		tabs = [
			{ id: 'alphaMods', label: 'Skater XL v1.2.2.8 (Alpha)', mods: alphaMods },
			{ id: 'publicMods', label: 'Skater XL v1.2.10.4 (Public)', mods: publicMods }
		];
